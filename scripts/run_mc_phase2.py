...
def main() -> None:
    with POSITION_PATH.open(encoding="utf-8") as f:
        psv = json.load(f)
    mean_pos = float(psv["mean_position_fraction"])
    ...
    trade_returns = pf.trades.records_readable["Return"].astype(float).to_numpy() * mean_pos
    ...
    payload = {
        ...
        "note": f"Dynamic position MC using mean_position_fraction={mean_pos:.3f} from R-based sizing",
    }
