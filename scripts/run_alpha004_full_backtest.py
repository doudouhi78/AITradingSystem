...
REPORT_PATH = ROOT / 'runtime' / 'alpha_research' / 'alpha004_full_backtest.json'
...
def main() -> None:
    ...
    signal_series = alpha101.alpha004(factor_input).rename('alpha004')
    signal_frame = signal_series.unstack('asset').reindex(index=prices.index, columns=prices.columns)

    suspended = market.pivot(index='trade_date', columns='symbol', values='is_suspended').reindex(index=prices.index, columns=prices.columns)
    suspended = suspended.astype('boolean').fillna(True).astype(bool)
