...
def run_manual_backtest(df: pd.DataFrame, entries: pd.Series, exits: pd.Series) -> dict[str, float | int | str]:
    ...
    return {
        "method": "手写纯Python（Sharpe按VectorBT口径）",
...
def main() -> None:
    ...
    result = run_manual_backtest(df, entries, exits)
